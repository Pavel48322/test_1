...
void main()
{
    ...
    float pX = ((gl_FragCoord.x - uPos.x + 40.0) / uResolution.x);
    float pY = ((gl_FragCoord.y - (uResolution.y - (uPos.y + uSizeImage.y))) / uResolution.y);
   
    float sx = uTime;
    float dy = (((0.1 * uScale.x) / (1.0 - (uScale.x - uScale.y))) / (10.0 * abs(pX - (pY / 6.0) - sx)));
    ...
    if ((pixel.r <= 0.2 && pixel.g <= 0.2 && pixel.b <= 0.2) || pixel.a < 0.1 ) {
        ...
        prov = vec4(pixel.r + 0.65, pixel.g + 0.65, pixel.b + 0.65, pixel.a);
...
export default class LasersLogo extends Phaser.Renderer.WebGL.Pipelines.PostFXPipeline
{
    constructor (game)
    {
        super({
    ...
    onPreRender ()
    {
        ...
        if (this.isStart) {
            ...
            if (this.loop > 1.15 * this.gameObject['scaleX']) {
                this.stopAnim()
            } else {
