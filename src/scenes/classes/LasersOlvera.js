...
void main()
{
    ...
    float pX = ((gl_FragCoord.x - uPos.x) / uResolution.x);
    float pY = ((gl_FragCoord.y - (uResolution.y - (uPos.y + uSizeImage.y))) / uResolution.y);
   
    float sx = uTime;
    float dy = ((0.1 * uScale.x) / (1.0 - (uScale.x - uScale.y))) / (20.0 * abs((pY / 4.0) + pX - sx));
    vec4 pixel = texture2D(uMainSampler, outTexCoord);

    vec4 prov;
    
    if (
        (pixel.r <= 0.2 && pixel.g <= 0.2 && pixel.b <= 0.2) 
        || pixel.a < 0.1 
        || gl_FragCoord.x - uPos.x > uSizeImage.x
    ) {
        prov = vec4(pixel.r, pixel.g, pixel.b, pixel.a);
    } else if (dy > 0.5) {
        prov = vec4(pixel.r + 0.55, pixel.g + 0.55, pixel.b + 0.55, pixel.a);
    } else {
...
export default class LasersOlvera extends Phaser.Renderer.WebGL.Pipelines.PostFXPipeline
{
    constructor (game)
    {
        super({
    ...
    onPreRender ()
    {
        if (!!this.gameObject && !this.isAddParam) {
            ...
            this.onCompleteCreate();
            this.isAddParam = true;
        }

        if (this.isStart) {
            this.set1f('uTime', this.loop);
            if (this.loop >= (0.38 * this.gameObject['scaleX']) && this.loop <= (0.381 * this.gameObject['scaleX'])/*0.45*/) {
                this.startStar()
                this.loop += 1 / (this.params.duraction / this.gameObject['scaleX']);
            } else if (this.loop > (0.45 * this.gameObject['scaleX'])) {
                this.loop = 0;
                this.stopAnim()
            } else {
                this.loop += 1 / (this.params.duraction / this.gameObject['scaleX']);
    ...
    startStar() {
        this.gameObject && this.gameObject.scene.events.emit("startStar")
    }
