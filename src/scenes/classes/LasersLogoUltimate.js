...
void main()
{
    ...
    float pY = ((gl_FragCoord.y - (uResolution.y - (uPos.y + uSizeImage.y))) / uResolution.y) + 0.2;
   
    float sx = uTime;
    float dy = (((0.1 * uScale.x) / (1.0 - (uScale.x - uScale.y))) / (10.0 * abs((pY / 3.0) - sx)));

    vec4 pixel = texture2D(uMainSampler, outTexCoord);

    vec4 prov;
    sx = 1.0;
    if ((pixel.r <= 0.5 && pixel.g <= 0.5 && pixel.b <= 0.5) || pixel.a < 0.9 ) {
        prov = vec4(pixel.r, pixel.g, pixel.b, pixel.a);
    }  else if (dy >= 0.2) {
        prov = vec4(pixel.r + 0.2, pixel.g + 0.2, pixel.b + 0.2, pixel.a);
    } else if (dy > 0.0 && dy <= 0.2) {
...
export default class LasersLogoUltimate extends Phaser.Renderer.WebGL.Pipelines.PostFXPipeline
{
    constructor (game)
    {
        super({
    ...
    onPreRender ()
    {
        ...
        if (this.isStart) {
            ...
            if (this.loop > 1 * this.gameObject['scaleX']) {
                this.stopAnim()
            }  else {
